...
const statusStyle: Record<string, { badge: string; icon: string; gradient: string; border: string }> = {
  building: {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {currentlyBuilding.map((item, i) => {
          ...
          return (
            <motion.div
              key={item.id}
              className={`rounded-xl border border-border border-l-2 ${style.border} bg-gradient-to-br ${style.gradient} p-5 flex flex-col gap-3 hover:shadow-sm transition-shadow duration-200`}
